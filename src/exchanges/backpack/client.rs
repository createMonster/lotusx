...
impl BackpackConnector {
    pub fn new(config: ExchangeConfig) -> Result<Self, ExchangeError> {
        ...
        let (signing_key, verifying_key) = {
            ...
            if secret_bytes.len() != 32 {
                return Err(ExchangeError::AuthError("Secret key must be 32 bytes".to_string()));
            }
    ...
    pub(crate) fn create_signed_headers(
        ...
        let api_key = self.verifying_key.as_ref()
        ...
        headers.insert("X-API-Key".to_string(), base64::engine::general_purpose::STANDARD.encode(api_key.to_bytes()));
