...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match ExchangeConfig::from_env_file("BACKPACK") {
        Ok(config) => {
            match run_private_streams(config).await {
    ...
    println!("\n📋 Summary:");
    println!("   • Public streams: ticker, bookTicker, depth, trade, kline, markPrice, openInterest");
        ...
        match msg? {
            Message::Text(text) => {
                ...
                if let Ok(data) = serde_json::from_str::<Value>(&text) {
                    ...
                    if message_count >= 20 {
                        println!("📈 Received {} messages, stopping public streams...", message_count);
                        break;
                    }
                }
            }
        ...
        match event_type {
            "ticker" => {
                ...
                if let (Some(symbol), Some(last_price), Some(volume)) = (
                    ...
                    println!("📊 #{:02} Ticker: {} Last: ${} High: ${} Low: ${} Vol: {}", 
            ...
            "depth" => {
                ...
                if let Some(symbol) = data.get("s").and_then(|s| s.as_str()) {
                    let asks_count = data.get("a").and_then(|a| a.as_array()).map(|a| a.len()).unwrap_or(0);
                    let bids_count = data.get("b").and_then(|b| b.as_array()).map(|b| b.len()).unwrap_or(0);
                    let update_id = data.get("u").and_then(|u| u.as_str()).unwrap_or("N/A");
                    println!("📋 #{:02} Depth: {} Updates: {} ({} asks, {} bids)", 
            ...
            "markPrice" => {
                ...
                if let (Some(symbol), Some(mark_price)) = (
                    ...
                    println!("💰 #{:02} MarkPrice: {} Mark: ${} Index: ${} Funding: {}%", 
            ...
            "openInterest" => {
                ...
                if let (Some(symbol), Some(open_interest)) = (
                    ...
                    println!("📊 #{:02} OpenInterest: {} {}", count, symbol, open_interest);
                }
            }
            _ => {
...
async fn run_private_streams(config: ExchangeConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (ws_stream, _response) = match connect_async(ws_url).await {
        Ok((stream, response)) => {
            println!("✅ Authenticated connection successful, status: {}", response.status());
            (stream, response)
        }
        Err(e) => {
        ...
        match msg? {
            Message::Text(text) => {
                ...
                if let Ok(data) = serde_json::from_str::<Value>(&text) {
                    ...
                    // Limit output for demo purposes
                    if message_count >= 10 {
                        println!("🔐 Received {} private messages, stopping...", message_count);
                        break;
                    }
                }
            }
...
fn handle_private_message(data: Value, count: usize) {
    if let Some(event_type) = data.get("e").and_then(|e| e.as_str()) {
        match event_type {
            "orderUpdate" => {
                ...
                if let (Some(symbol), Some(side), Some(status)) = (
                    ...
                    println!("📋 #{:02} OrderUpdate: {} {} {} @ {} Status: {} ID: {}", 
        ...
        println!("✅ Private subscription confirmed: {:?}", data.get("result"));
    } else if data.get("error").is_some() {
